...
<div class="hero-container">
    <div class="status-badge">?? Engine Status: Online ? Multi-Agent Pipeline Ready</div>
...
tab1, tab2, tab3, tab4 = st.tabs([
    "?? Module 1: Tableau YoY Drop Engine",
    "?? Module 2: AI Copy Critic & PAS Re-writer",
...
# -------------------------------------------------------------
with tab1:
    st.markdown("### ?? D2C Tableau Campaign Ingestion & YoY Variance Engine")
    ...
    with col_chart:
        ...
        fig.add_trace(go.Scatter(
            x=df_d2c['campaign_name'],
            y=df_d2c['vip_signups'] * 100,  # Scaled for secondary view
            name='VIP Signups (Index)',
            yaxis='y',
            mode='lines+markers',
    ...
    with col_table:
        st.markdown("#### ?? Raw Drop Records")
        st.dataframe(df_d2c[['campaign_name', 'channel', 'vip_signups', 'revenue', 'aov', 'unsubscribe_rate']], use_container_width=True, height=340)

    st.divider()

    st.markdown("### ?? Multi-Agent Drop Post-Mortem & Creative Hook Engine")
    
    if st.button("?? Execute 3-Agent YoY Post-Mortem", key="btn_m1", type="primary"):
...
### ?? AGENT 3: CREATIVE HOOK & VIP LIFECYCLE AGENT
...
#### ?? High-Converting SMS Variations for Upcoming Q3 Drop:
1. `?? VIP VAULT: Your private Q3 drop is officially unlocked, [First Name]. 2 free limited edition pieces reserved in your bag for 24h: [Link] (Txt STOP to opt out)`
2. `?? Secret Restock: Members get 1st pass on the sold-out Lace Capsule before public launch tomorrow at 9 AM: [Link]`
3. `?? You're on the list. Exclusive VIP early access starts right now. Sizes go fast: [Link]`

#### ?? High-Converting Email Sequences:
* **Subject:** `VIP First Pass: Your secret capsule is unlocked ??`  
...
# -------------------------------------------------------------
with tab2:
    st.markdown("### ?? Multi-Agent A/B Test Post-Mortem & Copywriting Critic")
    ...
    c_ab1.metric("?? Winner", winner['variant'], f"{winner['ctr']:.2f}% CTR (+{winner['ctr']-loser['ctr']:.2f}%)")
    c_ab2.metric("?? Underperformer", loser['variant'], f"{loser['ctr']:.2f}% CTR")
    c_ab3.metric("?? Statistical Confidence", f"{confidence:.1f}%", "Statistically Significant (p < 0.001)")
    ...
    with col_v1:
        ...
        <div class="critique-card-bad">
            ...
            <span class="pill-tag" style="background:#fecdd3; color:#9f1239;">CTA: ?? Weak</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_v2:
        ...
        <div class="critique-card-good">
            ...
            <hr>
            <span class="pill-tag" style="background:#bbf7d0; color:#166534;">Hook: ?? Pain-Point / Curiosity</span>
    ...
    st.markdown("### ?? Multi-Agent Copywriting Teardown & PAS Re-writer")
    
    if st.button("?? Run Copywriting Critic & PAS/AIDA Rewrite", key="btn_m2", type="primary"):
        if not api_key and not demo_mode:
            st.error("Please enter an OpenAI API Key or toggle '? Use Instant Demo Cache' in the sidebar.")
        elif demo_mode and not api_key:
            with st.spinner("Critic agent reviewing copy mechanics and generating revisions..."):
                time.sleep(1)
                st.markdown("""
### ?? 1. ANALYST AGENT: Performance Attribution
...
### ?? 2. CRITIC AGENT: Teardown of Variant A (The Loser)
...
### ?? 3. COPYWRITER AGENT: 3 High-Converting Re-writes (PAS Framework)

#### Option 1: The VIP Entitlement Angle (PAS)
* **Subject Line:** `Your VIP box just got upgraded (2 free pieces inside) ??`
...
* **CTA Button:** `[ Unlock My Curated Capsule ? ]`
...
# -------------------------------------------------------------
with tab3:
    st.markdown("### ?? Machine-Learning RFM Customer Segmentation & Churn Risk")
    ...
    if os.path.exists(rfm_file):
        ...
        def label_persona(row):
            if row['Frequency'] >= rfm_df['Frequency'].quantile(0.75) and row['Monetary'] >= rfm_df['Monetary'].quantile(0.75):
                ...
                return "?? At-Risk VIP (Churn Hazard)"
            else:
        ...
        with col_rfm_chart:
            fig_rfm = px.scatter(
                ...
                color_discrete_map={
                    ...
                    "? Loyal Active": "#10b981",
                    "?? At-Risk VIP (Churn Hazard)": "#f43f5e",
                    "?? Hibernating / Low-Value": "#94a3b8"
                },
        ...
        with col_rfm_stats:
            st.markdown("#### ?? Segment Lifecycle Metrics")
        ...
        st.markdown("### ?? Generated Twenty CRM / Klaviyo Real-Time Webhook Payloads")
...
# -------------------------------------------------------------
with tab4:
    st.markdown("### ??? Multi-Channel SMS & Push Notification Fatigue Guard")
    ...
    customers = pd.DataFrame([
        {"customer_id": "C_101", "name": "Elena Rostova", "vip_tier": "?? Diamond", "messages_last_7d": 1, "unsub_risk_score": 0.12, "last_msg_hours_ago": 72},
        {"customer_id": "C_102", "name": "Marcus Vance", "vip_tier": "Standard", "messages_last_7d": 5, "unsub_risk_score": 0.78, "last_msg_hours_ago": 6},
        {"customer_id": "C_103", "name": "Amina Chen", "vip_tier": "?? Gold", "messages_last_7d": 3, "unsub_risk_score": 0.45, "last_msg_hours_ago": 28},
    ...
    for idx, row in customers.iterrows():
        if row['unsub_risk_score'] > 0.70:
            ...
            status, reason = "?? SUPPRESSED", "< 24h Cooling Period Active"
        elif row['messages_last_7d'] >= (5 if "Diamond" in row['vip_tier'] or "Gold" in row['vip_tier'] else 3):
            status, reason = "?? SUPPRESSED", "Weekly Frequency Cap Exceeded"
        else:
